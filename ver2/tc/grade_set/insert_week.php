...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($conn->query($sql_insert) === TRUE && $conn->query($sql_create_table) === TRUE) {
        ...
        while ($row = $result_stu->fetch_assoc()) {
            ...
            $stu2 = $row["seatNum"];
            $stu3 = $row["name"];
            $sql = "INSERT INTO `$id_new`(`classNum`, `seatNum`, `name`, `SUM`, `ave`, `gradeRank`, `gradeRankPer`, `classRank`, `classRankPer`,`sign`) VALUES ('$stu1', '$stu2', '$stu3','0','0','0','0','0','0','');";
            $conn->query($sql);
        }

    }
    $_SESSION["week_set_ID"] = $id_new;
    $_SESSION["week_set_choose"] = "week編輯";
